...
/* Turn Right
 * This method will turn the robotic car to right
 */
void turnright()
{
    isTurning = 1;
...
/* Turn Left
 * This method will turn the robotic to the left
 */
void turnleft()
{
    isTurning = 1;
    // Set the right motor to move clockwise
    // Set P4.5 and 4.4 as out for right wheel to move it clockwise High on In2 and Low on In1
    GPIO_setOutputLowOnPin(GPIO_PORT_P4, GPIO_PIN2);  // In4
    GPIO_setOutputLowOnPin(GPIO_PORT_P4, GPIO_PIN0);  // In3
    GPIO_setOutputHighOnPin(GPIO_PORT_P4, GPIO_PIN5); // In2
    GPIO_setOutputLowOnPin(GPIO_PORT_P4, GPIO_PIN4);  // In1
...
/* Rotate
* This method will rotate the car 360 degree clockwise
* Input: None
* Output: None
*/
void rotate() {
...
/* Port1 ISR - This ISR will stop the motor when pressed and move the robotic car forward
* P1.1 -> Stop
* P1.4 -> Forward
*/
...
/*
*  Function: Adjust the PWM by using the PID controller using Kp, Ki and Kd
*  Input: int target
*  Output: None
*/
void CarPID(int target)
{
    ...
    leftcount = 0;
    rightcount = 0;
    // Dont need to reajust the PWM if the error is less than 3.
